...
export class APIClient {
  ...
  private url = (subPath?: string) => (subPath ? `${this.endpoint}/${subPath}` : this.endpoint);

  get = <T>(subPath?: string, config?: AxiosRequestConfig) =>
    httpClient.get<T>(this.url(subPath), config).then((r) => r.data);

  post = <T>(data?: unknown, subPath?: string) => httpClient.post<T>(this.url(subPath), data).then((r) => r.data);

  patch = <T>(data?: unknown, subPath?: string) => httpClient.patch<T>(this.url(subPath), data).then((r) => r.data);
